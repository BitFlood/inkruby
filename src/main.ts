...
export default class PoemPlugin extends Plugin {
	// 代码块类型配置映射：代码块标识符 -> 对应的CSS行类名
	private readonly BLOCK_CONFIG = {
		poetry: 'poetry-line', // poetry代码块使用poetry-line类名
		lc: 'lc-line', // lc代码块使用lc-line类名
		LC: 'lc-line', // LC代码块同样使用lc-line类名
	} as const;
	...
	async onload(): Promise<void> {
		// 遍历配置对象，为每种代码块类型注册处理器
		Object.entries(this.BLOCK_CONFIG).forEach(([blockType, lineClass]) => {
			this.registerMarkdownCodeBlockProcessor(
				blockType,
				(source, el, ctx) => {
					this.handleCodeBlockRendering(lineClass, source, el, ctx);
				},
			);
		});
	}
	 ...
	 */
	private handleCodeBlockRendering(
		lineClass: string,
		source: string,
